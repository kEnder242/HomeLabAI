...
def list_cabinet() -> str:
    """The Filing Cabinet: Lists all high-fidelity technical artifacts."""
    try:
...
def read_document(filename: str) -> str:
    ...
    if os.path.exists(path):
        with open(path, 'r') as f:
            return f.read()
    # Check general field notes (slug match)
    slug_path = os.path.join(WORKSPACE_DIR, f"field_notes/{filename}")
